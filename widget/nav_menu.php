...
<?php 	$menus = wp_get_nav_menus( array( 'orderby' => 'name' ) ); ?>
 ...
 <p>   
   ...
   <label for="<?php echo $this->get_field_id('nav_style'); ?>">显示模式</label>
			<select id="<?php echo $this->get_field_id('nav_style'); ?>" name="<?php echo $this->get_field_name('nav_style'); ?>">
				<option value="0">默认一行2个</option>
	...
	$nav_style=apply_filters('widget_title', empty($instance['nav_style']) ? __('') : $instance['nav_style']);
		if($nav_style) {$nav_styles='nav_onelien';}
		?>
...
<div class="nav_menu_widgetss box">
   ...
   <?php  ob_start(); wp_nav_menu(array( 'walker' => new header_menu(),'container' => false,'menu' => $nav_menu ,'items_wrap' => '<ul id="menu_widgetss" class="menu_widgetss '.$nav_styles.'">%3$s</ul>' ) ); ?>
</div>
